...
void flash_init(void)
{
	if(flash_read_byte(FLASH_START_ADDR) != FLASH_PAGE0_HEADER_BYTE_VAL)
	{
		...
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, FLASH_START_ADDR, 0x0A);
		...
		if( ((*(volatile uint8_t*)(FLASH_START_ADDR)) == FLASH_PAGE0_HEADER_BYTE_VAL)  &&
		    ((*(volatile uint32_t*)(FLASH_USER_MEM_START_ADDR)) == FLASH_USER_MEM_SPACE_ADDR))
		{
